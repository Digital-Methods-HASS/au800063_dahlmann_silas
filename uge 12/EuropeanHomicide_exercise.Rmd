...
Western_Europe %>% 
  ...
  geom_line(lwd=1)+
  labs(title = "Homicide rates in Europe",
         x = "Year",
...
ggsave("homicides6.png", width = 16, height = 10, dpi = 150)
```
...
```{r Looking at trends in duration of reign among Danish rulers, eval = TRUE}
...
kings %>% 
  mutate(duration=Regeringsslut - Regeringsstart) %>% 
  mutate(midyear = Regeringsslut - duration/2) %>% 
  ggplot(aes(x=midyear, y=duration))+
  geom_point(col="red")+
  geom_smooth(col="pink")+
  labs(title = "Gennemsnitlig levetid for danske konger",
...
Assessing degrees of civilisation can be one of the big tasks when studying societal development over the last thousand years. A absolute claim to the idea of civilisation has through human history become a weapon of bestiality: Enslavement, imperial expansion and euthanasia are just a few of a myriad of horrifying examples of "superior" peoples claiming supremacy over others.
When studying the trends of homicide in Europe through the last 800 years, the data speaks for itself: the relative amount of homicides has been dropping steadily, with few exceptions. Comparing this to data on the reign of Danish monarchs in the same period, a similar pattern emerges: people live longer and kill less. 
I would be precaucios concluding based on this data, that people are necessarily more civilized today than in the Middle Ages, but one thing is certain: our societies have gradually been structured in ways where killing has become less acceptable, and where being in positions of power is no longer a highway to hell.
